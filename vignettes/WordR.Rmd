...
R language has many ways for producing state of the art reports. For example `rmarkdown` or `Sweave` packages are very effective for preparing reports in PDF format. However, such techniques has some drawbacks:

- For nontechnical person it is very difficult to prepare the source (template) file.
- It is not conveniently possible to further edit the resulting report.
...
- result is again editable MS Word document,
- such document can be later saved as PDF in MS Word.

To conclude, `WordR` package is usefull, for example when you need to 
...
All examples, and instructions given in this document applies to MS Word 2013. No major differencies are expected to other versions (as of 2017).

### Template preparation
First we need to create a template (.docx) file. Such file may contain any formatting MS Word allows. Apart from that the template can contain two other things:

- inline R code
- bookmarks for inserting Tables or Plots

#### Creating inline R code
This functionality enables to include simple R expressions anywhere in the Word document, which is evaluated during file rendering. The result of the epxression need to be a string of length one. Because of limitation in `officer` package, line breaks does not works. Because of used workflow, functions etc. the R expression need to be a separate paragraph. However, MS Word offers a way how to do an inline paragraph.
Steps for creating an inline expression:

- Open MS Word, write some text within
- locate the cursor on the place where you want to have the R expression
- insert style separator by pressing `Ctrl+Alt+Enter` (it is preferable to have formatting symbols visible by clicking the "paragraph" sign button on `Home>Paragraph` panel in MS Word)
- delete the space which MS Word inserted after the style separator
- type `rXXXX expression @{style}r#`, where XXXX is a numeric unique code block identificator (must be unique across whole document), expression is R expression like `1+1`. Last part `@{style}` is optional argument specifying the MS Word style in which the expression should be evaluated (for details see help for function `renderInlineCode`).
...
Example can be seen in `examples/examples.R`.
